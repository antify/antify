...
export const useGuard = (rawToken: string | null) => {
    ...
    return {
        ...
        hasPermissionTo: (permission: string[] | string, tenantId: string) => {
            ...
            const tenantAccess = token.tenantsAccess.find(tenantAccessItem => tenantAccessItem.tenantId === tenantId);
            ...
            if (Array.isArray(permission)) {
                return tenantAccess.permissions.some(permissionItem => permission.some(permissionToFind => permissionToFind === permissionItem));
            }

            return tenantAccess.permissions.some(permissionItem => permissionItem === permission);
        }
    }
}