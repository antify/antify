...
import { authLoginPostValidator } from '~~/glue/api/auth/login.post';
...
const submit = async () => {
  console.log('RESET');
  validator.validateProperty('email', formData.value.email, 1);
...
<template>
  <div>
    ...
    <AntModal v-model:active="active" class="z-100">
      ...
      <template #buttons>
        <AntButton label="Schließen" @click="active = false" size="small" />

        <AntButton :primary="true" size="small">
