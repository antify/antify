...
import { validator as baseValidator } from '~~/glue/api/cockpit/mail_templates/[mailTemplateId].put';
...
const { data, refresh, error } = await useFetch(
  `/api/pages/cockpit/mail_templates/${useRoute().params.mailTemplateId}`,
  useDefaultFetchOpts()
);

mailTemplate.value = data.value.default;
...
async function onSubmit() {
  ...
  validator.value.validate(mailTemplate.value, 1);
...
async function onSendTestMail() {
  ...
  await useFetch(
    `/api/pages/cockpit/mail_templates/${
      useRoute().params.mailTemplateId
    }/send_test_mail`,
    {
      ...useDefaultFetchOpts(),
      method: 'POST',
...
<template>
  <div>
    <AntDualContent>
      ...
      <template #mainBody>
        ...
        >
          <div class="transition-opacity space-y-4 mt-0">
            <div data-cy="title">
              <AntInput
                v-model:value="mailTemplate.title"
                label="Bezeichnung"
                :errors="validator.errorMap['title']"
                :validator="
                  () =>
                    validator.validateProperty('title', mailTemplate.title, 1)
                "
                :loading="loading"
                :disabled="saving"
              >
            ...
            >
              <AntRichTextEditor
                v-model:data="mailTemplates.content"
                label="Inhalt"
      ...
      <template #mainFooter>
        <AntButton>
          <NuxtLink :to="{ name: 'cockpit-mail-templates' }"> Zurück </NuxtLink>
        </AntButton>
